...
class MusicPlayer:
    ...
    def basic_setup(self):
        # Heading
        Label(self.window,text="Generation Music Player", font=("Arial",20,"bold","italic"),bg="Hotpink4",fg="yellow").place(x=240,y=25)
        ...
        self.my_list_song = Listbox(frame, bg="Hotpink4", fg="yellow", width=120, height=10, font=("Arial",8,"bold"), relief=SUNKEN, borderwidth=20, yscrollcommand=v_scroll.set)
    ...
    def instructional_btn_setup(self): # Some button initialization
        song_add = Button(self.window,text="Add Song",font=("Helvetica",15,"bold","italic"), bg="Hotpink4",fg="yellow", activebackground="turquoise1", activeforeground="Black", width=12, height=1, relief=RAISED, borderwidth=5, command=self.add_song)
        song_add.place(x=480,y=352)

        delete_song = Button(self.window, text="Delete Selected Song", font=("Helvetica", 14, "bold", "italic"), bg="Hotpink4", fg="yellow", activebackground="turquoise1", activeforeground="Black", width= 27, relief=RAISED, borderwidth=5,command=self.delete_selected_song)
        delete_song.place(x=480, y=437)

        song_counter = Button(self.window, text="Song Counter", width=13,font=("Helvetica", 15, "bold", "italic"), activebackground="turquoise1", activeforeground="yellow", bg="Hotpink4", fg="yellow", relief=RAISED, borderwidth=5, command=self.song_counter)
    ...
    def song_duration(self):# Make Song_Duration Label
        self.song_duration_bar = Label(self.window, text="Song Duration", font=("Arial",17,"bold"), fg="yellow", bg="Hotpink4",width=25)
    ...
    def muter(self):# Mute set-up
        self.mute_scale = Scale(self.window,from_=1,to=0,orient=HORIZONTAL,bg="Hotpink4",command=self.get_mute, activebackground="turquoise1",font=("Arial",15,"bold"),length=47,relief=RIDGE,bd=3)
        self.mute_scale.place(x=25,y=430)

        self.mute_scale.set(self.mute_status)

        mute_indicator = Label(self.window,text="Mute",font=("Arial",10,"bold"),fg="yellow",bg="Hotpink4")
    ...
    def repeat_controller(self):# Repeat Set-up
        self.loop_bar = Scale(self.window,from_=1,to=0,orient=HORIZONTAL,bg="Hotpink4",command=self.repeat_maintain, activebackground="turquoise1",font=("Arial",15,"bold"),length=140,relief=RIDGE,bd=3)
        self.loop_bar.place(x=115,y=430)

        self.loop_bar.set(self.repeat_status)

        loop_bar_indicator = Label(self.window,text="Off    Repeat    On",font=("Arial",10,"bold"),fg="yellow",bg="Hotpink4")
    ...
    def shuffle_controller(self):# Shuffle set-up
        self.shuffle_bar = Scale(self.window,from_=1,to=0,orient=HORIZONTAL,bg="Hotpink4",command=self.shuffle_maintain, activebackground="turquoise1",font=("Arial",15,"bold"),length=140,relief=RIDGE,bd=3)
        self.shuffle_bar.place(x=305,y=430)

        self.shuffle_bar.set(self.shuffle_status)

        shuffle_bar_indicator = Label(self.window,text="  Off    Shuffle    On",font=("Arial",10,"bold"),fg="yellow",bg="Hotpink4")
...
if __name__ == '__main__':
    ...
    window.config(bg="saddlebrown")

    backward_image_take = ImageTk.PhotoImage(Image.open('backward.png'))
    backward_btn_img = Button(window, image=backward_image_take, bg="Hotpink4", activebackground="turquoise1", relief=RAISED, bd=3)
    backward_btn_img.place(x=25,y=350)

    play_image_take = ImageTk.PhotoImage(Image.open('play.png').resize((40,40),Image.Resampling.LANCZOS))
    play_btn_img = Button(window,image=play_image_take,bg="Hotpink4", activebackground="turquoise1", relief=RAISED,bd=3)
    play_btn_img.place(x=115,y=350)

    pause_image_take = ImageTk.PhotoImage(Image.open('pause.png'))
    pause_btn_img = Button(window,image=pause_image_take,bg="Hotpink4", activebackground="turquoise1",relief=RAISED,bd=3)
    pause_btn_img.place(x=210,y=350)

    stop_image_take = ImageTk.PhotoImage(Image.open('stop_play_icon.png').resize((40,40),Image.Resampling.LANCZOS))
    stop_btn_img = Button(window,image=stop_image_take,bg="Hotpink4", activebackground="turquoise1", relief=RAISED,bd=3)
    stop_btn_img.place(x=305,y=350)

    forward_image_take = ImageTk.PhotoImage(Image.open('forward.png'))
    forward_btn_img = Button(window,image=forward_image_take,bg="Hotpink4", activebackground="turquoise1", relief=RAISED,bd=3)
